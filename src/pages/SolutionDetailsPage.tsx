...
export function SolutionDetailsPage() {
  ...
  if (isLoading) {
    return <p className="text-slate-600 dark:text-slate-300">Loading solution...</p>
  }
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{solution.title}</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {solution.categoryIds.map((categoryId) => (
          <span key={categoryId} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700 dark:bg-slate-700 dark:text-slate-200">
            {categoriesById.get(categoryId)?.name ?? 'Unknown'}
          </span>
        ))}
        {solution.categoryIds.length === 0 ? <p className="text-sm text-slate-500 dark:text-slate-400">No categories</p> : null}
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Estimation</h2>
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {Object.entries(solution.estimates).map(([key, value]) => (
            <div key={key} className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-600 dark:bg-slate-700">
              ...
              <p className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{value}h</p>
            </div>
          ))}
        </div>
      </div>
